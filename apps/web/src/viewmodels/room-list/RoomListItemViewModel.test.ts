...
vi.mock("../../RoomNotifs", async () => ({
    ...(await vi.importActual("../../RoomNotifs")),
    determineUnreadState: vi.fn(() => ({ level: 1, symbol: null, count: 0, invited: false })),
}));
...
describe("RoomListItemViewModel", () => {
    ...
    describe("Actions", () => {
        ...
        it("should dispatch view room and show thread actions on openThread", () => {
            ...
            vi.spyOn(room, "getThread").mockReturnValue({ rootEvent } as any);
        ...
        it("should not dispatch when the thread root cannot be found on openThread", () => {
            vi.spyOn(room, "getThread").mockReturnValue(null);
    ...
    describe("Active threads", () => {
        ...
        beforeEach(() => {
            ...
            vi.spyOn(MessagePreviewStore.instance, "generatePreviewForEvent").mockImplementation(
        ...
        it("should only include threads active within the configured number of days", () => {
            const now = Date.now();
            vi.spyOn(room, "getThreads").mockReturnValue([
        ...
        it("should reflect the mention notification state of a thread", () => {
            ...
            vi.spyOn(room, "getThreads").mockReturnValue([makeThread("$t:server", now - 1 * DAY_MS, "Thread")]);
        ...
        it("should not include any threads when the setting is zero", () => {
            ...
            const now = Date.now();
            vi.spyOn(room, "getThreads").mockReturnValue([makeThread("$t:server", now, "Thread")]);
