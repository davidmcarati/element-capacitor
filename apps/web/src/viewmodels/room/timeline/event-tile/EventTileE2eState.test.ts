...
function mockDecryptionFailure(mxEvent: MatrixEvent, reason: DecryptionFailureCode): void {
    vi.spyOn(mxEvent, "isDecryptionFailure").mockReturnValue(true);
...
describe("EventTileE2eState", () => {
    ...
    it("suppresses the authenticity-not-guaranteed indicator even with a key forwarding user", () => {
        const mxEvent = makeEvent();
        vi.spyOn(mxEvent, "getKeyForwardingUser").mockReturnValue("@bob:example.org");
    ...
    it("does not show an unencrypted warning for state events in encrypted rooms", () => {
        const mxEvent = makeEvent();
        vi.spyOn(mxEvent, "isState").mockReturnValue(true);
